...
When the Hero includes a reservation-to-follow-up overview, each stage must include a short action or outcome, not only a noun label. Present the stages as one continuous journey with a clear start, connected handoffs, and an end state; do not rely on a row of disconnected equal cards. Use a numbered left-to-right route on desktop and a left-aligned top-to-bottom route on mobile. When a supporting visual materially improves comprehension, generate separate text-free desktop and mobile assets with built-in imagegen and keep every number, label, action, and handoff statement as selectable HTML.
