...
- Audit all six fixed sections before source review and again after rendering. Each section must answer a distinct visitor question; remove or move repeated explanations, journeys, and responsibility splits. `content/lp-copy.json` must contain a PASS `section_role_audit`, and both creative reviews must contain exactly one `SECTION_DUPLICATION_AUDIT: PASS` evidence entry naming all six section IDs. The pre-render and final validators enforce these records and block exact repeated explanatory text across sections.
