...
4. Write concise Japanese copy for all six sections. Treat submitted prose as source material rather than copying it into the Hero. Build the Hero from one eyebrow, one H1 benefit promise, one H2 mechanism, and the CTA. Count displayed characters excluding whitespace and line breaks: eyebrow 10–18 recommended / 22 maximum, H1 18–28 recommended / 32 maximum, H2 20–36 recommended / 44 maximum. Do not add a long Hero explanation. Rewrite naturally to preserve audience, primary benefit, and mechanism, moving detail to later sections instead of truncating it. Keep unverified claims out of public copy. Compare all six sections pairwise, assign one unique information responsibility to each, and record the result in `content/lp-copy.json#section_role_audit`.
5. Plan the visual assets before building the design. The Hero must use one of two layouts: (a) HTML copy on the left with a full-bleed imagegen visual filling the right side, or (b) centered HTML copy over a full-bleed imagegen background. The Hero scene itself must be generated with built-in `imagegen`; do not draw the scene, people, devices, or pictorial illustration in CSS. Generate a separate mobile Hero when a responsive crop cannot preserve the subject, contrast, and text-safe area. Generate a text-free imagegen background for `final-cta`, and decide for every other section whether a supporting image or background is needed. Record the decision and rationale in `design/assets-manifest.json`. Build the wireframe and full responsive design as local HTML/CSS with exact approved Japanese copy over the real assets. All headings, body copy, labels, buttons, and CTA text remain selectable HTML; generated images contain no copy. Use the shared sticky header and shared footer from the section contract. The footer repeats the header navigation, has no back-to-top link, and becomes a left-aligned vertical group on mobile. Do not insert a standalone scope, applicability, reassurance, or context strip between fixed sections. Integrate necessary unique copy into the one fixed section that owns it and omit redundant copy. Before adding a Hero overview, compare it with `solution` and `process`; omit the overview when it repeats either section. When retained, a Hero overview must pair each stage label with a short action or result and read as one continuous journey with a clear start, connected handoffs, and an end state; do not default to disconnected equal cards. Use a numbered left-to-right route on desktop and a left-aligned top-to-bottom route on mobile. When a workflow visual improves comprehension, generate separate text-free desktop and mobile assets with built-in imagegen, keep the semantic sequence as an ordered list, and record its safe areas and label mapping. Group important Japanese copy by semantic phrase and verify natural line endings at 1440px and 390px. Save structured specs for colors, typography, spacing, dimensions, crop positions, overlays, safe areas, mappings, and responsive behavior.
...
- Both creative reviews must block one-character orphan lines, particles or punctuation stranded on their own line, word-internal splits, and line breaks that separate a modifier from its meaning. Check both 1440px and 390px evidence.
- Both creative reviews must verify the Hero copy budget and confirm that brevity has not removed the audience, primary benefit, or mechanism.
- Both creative reviews must audit all six section roles for semantic overlap and record one `SECTION_DUPLICATION_AUDIT: PASS` evidence entry naming every section. Exact repeated explanatory blocks, missing copy-audit metadata, or missing review evidence fail the automated validator.
- The shared header structure is fixed and sticky. The shared footer repeats the brand and three header anchors, omits back-to-top, and stacks left on mobile. A draft LINE CTA stays blank and disabled without any public `準備中` notice.
